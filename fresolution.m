%% fre-soil water-solution，
%Input:Nlay-soil layers; omega-angle frequency; K_s-hydraulic conductivity n_0-saturated soil moisture content ,alpha-Garden parameter,q_s-steady flux ,q_p-fluctuation flux at z=0,L
%Output:qqp-fluctuation flux ,theta_p-fluctuation soil moisture,K_p-fluctuation unsaturated hydraulic conductivity
function [qqp,theta_p,K_p]=fresolution(Nlay,omega,K_s,n_0,alpha,q_s,q_p,L)
k_=zeros(1,Nlay);
Kc=zeros(1,Nlay);
...
for i_lay=1:Nlay
    Kirchhoff1(i_lay)=exp(lamber1(i_lay)*alpha(i_lay)*L(i_lay));% the continuity of h
    Kirchhoff2(i_lay)=exp(lamber2(i_lay)*alpha(i_lay)*L(i_lay));% the continuity of h
    fluxcoef1(i_lay)=(1-lamber1(i_lay))*Kirchhoff1(i_lay);  % the continuity of q
    fluxcoef2(i_lay)=(1-lamber2(i_lay))*Kirchhoff2(i_lay);  % the continuity of q
...
AA(2*Nlay,2*Nlay)=Kirchhoff2(Nlay); %free drainage boundary
%DD(2*Nlay,Nlay)=1;
%EE(1)=DE;
BB(1)=BC;
CC=AA\BB;
...
dep=linspace(0.00,L(Nlay),L(Nlay)/0.01+1);% output location of depth
for kk=1:(L(1)/0.01+1)
%    K_c(kk)=GG(1)/alpha(1)*exp(alpha(1)*deep(kk))+GG(1+Nlay);
    K_p(kk)=CC(1)*exp(lamber1(1)*alpha(1)*dep(kk))+CC(1+Nlay)*exp(lamber2(1)*alpha(1)*dep(kk));
    qqp(kk)=(1-lamber1(1))*CC(1)*exp(lamber1(1)*alpha(1)*dep(kk))+(1-lamber2(1))*CC(1+Nlay)*exp(lamber2(1)*alpha(1)*dep(kk));
%    theta_s(kk)=n_0(1).*(K_c(kk)/K_s(1));
    theta_p(kk)=n_0(1).*(K_p(kk)/K_s(1));
end
    for i_lay=2:Nlay
        for kk=1:(L(Nlay)/0.01+1)
        if dep(kk)<=L(i_lay)&&dep(kk)>L(i_lay-1)
%        K_c(kk)=GG(i_lay)/alpha(i_lay)*exp(alpha(i_lay)*deep(kk))+GG(i_lay+Nlay);
        K_p(kk)=CC(i_lay)*exp(lamber1(i_lay)*alpha(i_lay)*dep(kk))+CC(i_lay+Nlay)*exp(lamber2(i_lay)*alpha(i_lay)*dep(kk));
        qqp(kk)=(1-lamber1(i_lay))*CC(i_lay)*exp(lamber1(i_lay)*alpha(i_lay)*dep(kk))+(1-lamber2(i_lay))*CC(i_lay+Nlay)*exp(lamber2(i_lay)*alpha(i_lay)*dep(kk));

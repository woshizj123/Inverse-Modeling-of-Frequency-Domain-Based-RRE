...
for jj=1:Njtime
for i=1:Nfobs
[~,comres(i,:),~]=fresolution(Nlay,2*pi*f(numbf(i)),exp(ksrenew(:,jj)),n_0,alpha,q_s,qp(numbf(i)),L);%选取频率进行计算
end
Am1=angle(comres);
...
for i=1:Nfobs
[~,comres1(i,:),~]=fresolution(Nlay,2*pi*f(numbf(i)),exp(ksdelta),n_0,alpha,q_s,qp(numbf(i)),L);%选取频率进行计算
end
Amm1=angle(comres1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for i=1:Nfobs
[~,comres2(i,:),~]=fresolution(Nlay,2*pi*f(numbf(i)),exp(ksrenew(:,jj+1)),n_0,alpha,q_s,qp(numbf(i)),L);%选取频率进行计算
end
Am2=angle(comres2);
